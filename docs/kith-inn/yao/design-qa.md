...
- 隔天提醒：时间分隔显示 `9月9日 12:36`，桃子直接发送第二张卡片，展示“午饭已送达 · 请付款”、`¥60`、餐次和订单号，不再额外发送重复的送达气泡。
- 工程对齐：这个展示场景对应工程师原型桃子端的手动补单能力；卡片同时承担顾客下一次进入自主下单流程的入口。
- 浏览器验证：总览页实测 `10` 张餐次卡且无横向溢出；私聊页标题为“桃子”，包含 `1` 条顾客下单消息、`1` 条桃子确认消息和 `2` 张应用卡片，消息区无横向或纵向溢出，浏览器错误为 `0`。
