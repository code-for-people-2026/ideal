...
- 交互对齐：默认选中周一午饭第一道菜，并在整张表下方显示菜名、分类、近期重复和费工度。前三次点击“换这道”由系统在同类菜中优先选择本周未出现且该格未试过的菜；三次用完后按钮才变为“自己挑一道”，打开完整同类候选列表。
- 视觉证据：星期单表页面 `/tmp/kith-menu-generation-v6/menu-weekday-table.png`；三次智能更换后的自选状态 `/tmp/kith-menu-generation-v7/smart-swap-after-three.png`。
...
- 比较历史：Pass 1 把参考表格藏在展开层级中；Pass 2 又擅自改成十张纵向餐次卡；Pass 3 回到参考表格但拆为午餐、晚餐两张；Pass 4 合并为一张十行表；Pass 5 将行首从具体日期改为周一至周五，并对调星期与餐次字号；Pass 6 修正“点击即自选”的流程错误，改为三次规则推荐后再开放自选。当前保留参考图的五类菜品、选中描边、详情和换菜交互，未发现剩余 P0 / P1 / P2。
