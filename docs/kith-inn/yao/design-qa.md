...
- 结构核对：不再拆成午餐、晚餐两张表；改为一张 `餐次 / 荤1 / 荤2 / 素1 / 素2 / 汤` 六列表，十行依次为 `3号午饭、3号晚饭、4号午饭、4号晚饭……7号晚饭`，完整覆盖十个餐次和五十个菜品按钮。
- 标题层级：每行以午饭或晚饭为主要标签，日期用小字号放在其上方，既保持按日期阅读的顺序，也落实“日期是二级标题”的产品要求。
...
- 交互对齐：默认选中 3 号午饭第一道菜，并在整张表下方显示菜名、分类、近期重复、费工度和“换这道”；点击任意餐次的菜品后，选中描边和详情同步切换，同类候选替换逻辑保持不变。
- 视觉证据：最终单表页面 `/tmp/kith-menu-generation-v5/menu-single-table.png`。
- 浏览器验证：DOM 实测 `1` 张周表、`10` 个餐次行、`50` 个菜品按钮、`1` 个默认选中状态和 `1` 个详情面板；前四行标签依次为 `3号午饭、3号晚饭、4号午饭、4号晚饭`。切换到 3 号晚饭“荷塘小炒”后，选中项与详情一致，且“换这道”可打开 `3` 个同类候选。
- 比较历史：Pass 1 把参考表格藏在展开层级中；Pass 2 又擅自改成十张纵向餐次卡；Pass 3 回到参考表格但拆为午餐、晚餐两张。Pass 4 按最新要求合并为一张十行表，保留参考图的五类菜品、选中描边、详情和换菜交互，未发现剩余 P0 / P1 / P2。
