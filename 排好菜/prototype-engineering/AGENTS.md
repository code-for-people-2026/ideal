...
For this prototype, mirror Kith-Inn's desktop-pointer behavior inside the phone: use the native arrow/pointer/grab cursors and hide the mobile runtime's translucent touch cursor. Keep this override app-scoped in `src/prototype.css` so the shared runtime remains intact.

Match Kith-Inn's generation-page color hierarchy: the rule card uses light warm gold `#fff2d5` with brown copy `#765313`, while brick red remains reserved for the primary generation action.

All four primary bottom-navigation destinations must be operable in leadership review builds. `排菜单`, `菜品库`, `历史`, and `我的` use shareable query-string routes; never leave a visible primary tab as inert chrome.
