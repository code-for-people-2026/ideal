...
- Keep `page-map.html` and `engineering-model.html` as secondary views linked from the runnable prototype. Do not replace the runnable default page with either canvas.
...
- Keep the Vite multi-page inputs for `index.html`, `page-map.html`, and `engineering-model.html` so all three views deploy under the same GitHub Pages path.
