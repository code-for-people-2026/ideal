...
近邻互助组探索已有邻里关系如何沉淀为可信、可持续的互助网络。2026-09-02，当前业务范围进一步收缩到 [3.1 近邻闲置](../3.1-neighborhood-exchange/README.md)；本目录不是废弃区，而是后续扩大范围时必须回看的产品基线。
...
这套层级只解释 3.2，不会自动覆盖当前 3.1 的范围。原型不是生产代码，也不能替代新的技术设计、接口契约和自动化验收。

## 重新启动的条件

只有当近邻闲置已经证明邻里流转能够持续发生，而且用户明确提出超出物品流转的互助需求，才重新评估 3.2。重启时应先写新决议，说明哪些旧结论继续成立、哪些需要作废。
...
```text
http://127.0.0.1:4173/3.2-neighborhood-mutual-aid-team/
```
