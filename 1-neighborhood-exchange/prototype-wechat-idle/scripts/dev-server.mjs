...
const types = { '.html': 'text/html; charset=utf-8', '.js': 'text/javascript; charset=utf-8', '.css': 'text/css', '.json': 'application/json', '.jpg': 'image/jpeg' };
export function startServer(port = Number(process.env.PORT || 4187)) {
  const server = http.createServer(async (req, res) => {
    try {
      ...
      if (['/api/catalog', '/api/access', '/api/media'].includes(url.pathname)) {
      ...
      if (!['/', '/index.html', '/baolong', '/luanshan', '/app.js', '/style.css'].includes(url.pathname)) { res.writeHead(404).end(); return; }
      const file = resolve(root, '.' + decodeURIComponent(['/', '/baolong', '/luanshan'].includes(url.pathname) ? '/index.html' : url.pathname));
