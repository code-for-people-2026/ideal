...
export default {
  async fetch(request) {
    ...
    try {
      ...
      if (typeof body?.password !== 'string' || !body.password.length || body.password.length > 128) return respond('INVALID_INPUT', 400);
      if (!await allowLogin(request, group)) return respond('TOO_MANY_ATTEMPTS', 429, { 'Retry-After': '900' });
      if (!await checkPassword(group, body.password)) return respond('INCORRECT_PASSWORD', 401);
