...
- 右上角「分享」为当前日期生成白底长图，包含客户、分量、金额、付款状态和底部二维码，支持预览、保存及系统分享。二维码使用正式域名，携带本群凭证和日期，扫码直接查看对应账单。
- 每次完成数据同步后，从云端比较订单及人工确认变化，为有变化的群和日期生成分享图，通过本机 computer-use 发送至文件传输助手；发送结果保存在私有运行目录，无变化不重复发送。
...
`snapshot*.json`、采集与导出目录、数据库文件、分享图片目录、发送状态和完整访问链接均属于私有运行数据。配置模板 `sync.config.example.json` 与依赖清单正常入库；不要把真实值填入模板。仓库检查同时覆盖 PR 新增提交，防止私有文件先被上传再在后续提交删除。新 PR 的提交中没有旧快照，不代表其他旧分支或历史提交已被清除。
