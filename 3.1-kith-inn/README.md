...
[群订单对账工具](./order-ledger/README.md) 与上述展示原型分开维护。它独立部署在 Vercel，使用 Neon 存储经授权整理的群订单及人工确认，需要各群专属链接访问。源代码保存在本仓库，原始微信消息、访问密钥和本机配置留在仓库外；不参与微信支付，也不改变现有产品 P0 范围。旧的 GitHub Pages 对账入口已停用，该应用不进入 Pages 产物。
